...
* Understand different approaches to scheduling (online vs. offline, static vs. dynamic) and migration  (offline/online, task-level, job-level, free)
...
* Identify jobs with similar properties and allocate these to the same elements of the technical solution
...
* Understand that the concepts for using interrupts at the application level will generally depend on the processor architecture (e.g. number of priorities) and the RTOS (support for prologue/epilogue mechanism).
...
* Understand that the precision and accuracy of WCET analysis is limited and is influenced by the complexity of the hardware and software (impact of caches, pipelines, shortest vs. longest program path, memory management, dynamic dispatch, ...)
